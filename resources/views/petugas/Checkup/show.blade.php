...
<div class="checkup-wrapper">
    <div class="checkup-container">
        ...
        <div class="btn-action-area" style="margin-top: 50px; text-align: center; display: flex; justify-content: center; gap: 15px;">
            <a href="{{ route('petugas.checkup.index') }}" class="btn-modern-back" style="background: #64748b;">
                ← Kembali
            </a>

            @if($document->canBeEdited())
                <a href="{{ route('petugas.checkup.edit', $document->id) }}" class="btn-modern-back" style="background: #f59e0b;">
                    Edit Data
                </a>

                <form action="{{ route('petugas.checkup.submit', $document->id) }}" method="POST" class="inline">
                    @csrf
                    <button type="submit" 
            ...
            @if($document->workflow_status == 'approved')
                <a href="{{ route('petugas.checkup.preview', $document->id) }}" target="_blank" class="btn-modern-back" style="background: #3b82f6;">
                    Download PDF
                </a>
            @endif
        </div>
