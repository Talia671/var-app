...
<div class="flex flex-col md:flex-row justify-between items-center mb-6">
    ...
    <div class="mt-4 md:mt-0">
        <a href="{{ route('petugas.simper.create') }}" 
           class="inline-flex items-center justify-center px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 shadow-md transition-all">
            ...
            <tbody class="bg-white dark:bg-night-card divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-150">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium flex items-center gap-2">
                        @if($item->canBeEdited())
                            <form action="{{ route('petugas.simper.submit', $item->id) }}" method="POST" class="inline">
                                @csrf
                                <button type="submit" 
                                        class="text-white bg-green-600 hover:bg-green-700 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-xs px-3 py-2 transition-colors shadow-sm"
