...
<div class="max-w-7xl mx-auto">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-xl font-bold text-gray-800">Detail RANMOR Kendaraan</h2>

        <div class="flex gap-2">
            @if($document->canBeEdited())
                <a href="{{ route('petugas.ranmor.edit',$document->id) }}"
            ...
            @endif
            <a href="{{ route('petugas.ranmor.index') }}" class="px-4 py-2 bg-gray-200 text-gray-800 rounded-lg hover:bg-gray-300 transition">
                Kembali
            </a>
        </div>
    </div>
    ...
    <div class="mb-6 flex items-center gap-3">
        <span class="text-sm font-medium text-gray-500 text-sm">Status Dokumen:</span>
        @if($document->workflow_status == 'draft')
            <span class="px-4 py-1.5 bg-gray-100 text-gray-800 rounded-full text-xs font-bold uppercase tracking-wider">Draft</span>
        @elseif($document->workflow_status == 'submitted')
            <span class="px-4 py-1.5 bg-yellow-100 text-yellow-800 rounded-full text-xs font-bold uppercase tracking-wider">Submitted</span>
        @elseif($document->workflow_status == 'approved')
            <span class="px-4 py-1.5 bg-green-100 text-green-800 rounded-full text-xs font-bold uppercase tracking-wider">Approved</span>
        @elseif($document->workflow_status == 'rejected')
            <span class="px-4 py-1.5 bg-red-100 text-red-800 rounded-full text-xs font-bold uppercase tracking-wider">Rejected</span>
        @endif
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="md:col-span-2 space-y-6">
            <div class="bg-white shadow rounded-xl p-6">
                <h3 class="font-semibold mb-6 border-b pb-3 text-blue-600 flex items-center gap-2">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
        ...
        <div class="space-y-6">
            <div class="bg-white shadow rounded-xl p-6">
                <h3 class="font-semibold mb-6 border-b pb-3 text-red-600 flex items-center gap-2">
