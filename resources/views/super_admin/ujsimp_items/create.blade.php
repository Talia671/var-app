...
<div class="mb-6">
    ...
    <p class="text-gray-500 dark:text-gray-400 text-sm mt-1">Tambah data item penilaian baru untuk UJSIMP</p>
</div>
...
<div class="bg-white dark:bg-night-card rounded-xl shadow-lg border border-gray-100 dark:border-night-border overflow-hidden p-6">
    <form action="{{ route('super-admin.ujsimp-items.store') }}" method="POST">
        @csrf
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            ...
            <div>
                ...
                <input type="number" name="urutan" id="urutan" value="{{ old('urutan') }}" required 
        ...
        <div class="mb-4">
            ...
            <textarea name="uraian" id="uraian" rows="3" required 
                      class="block w-full px-4 py-2 text-base border-gray-300 dark:border-gray-600 focus:outline-none focus:ring-secondary focus:border-secondary sm:text-sm rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white @error('uraian') border-red-500 @enderror">{{ old('uraian') }}</textarea>
            @error('uraian')
                <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
            @enderror
        </div>
