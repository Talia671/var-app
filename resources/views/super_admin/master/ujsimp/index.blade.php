...
<div class="flex justify-between items-center mb-6">
    <div>
        <h2 class="text-2xl font-bold text-gray-800 dark:text-white">Master Data UJSIMP</h2>
        <p class="text-gray-500 dark:text-gray-400 text-sm mt-1">Konfigurasi item penilaian UJSIMP</p>
    </div>
    <a href="{{ route('super-admin.master.ujsimp.create') }}" class="bg-primary hover:bg-primary-dark text-white font-medium rounded-lg text-sm px-4 py-2.5 transition-colors">
        Tambah Item
    </a>
</div>
            ...
            <tbody class="bg-white dark:bg-night-card divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium flex gap-2">
                        <a href="{{ route('super-admin.master.ujsimp.edit', $item->id) }}" class="text-blue-600 hover:text-blue-900">Edit</a>
                        <form action="{{ route('super-admin.master.ujsimp.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?')">
                            @csrf
                            @method('DELETE')
