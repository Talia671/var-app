...
<div class="max-w-4xl mx-auto">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h2 class="text-2xl font-bold text-gray-800 dark:text-white">Tambah Item UJSIMP</h2>
        </div>
        <a href="{{ route('super-admin.master.ujsimp.index') }}" class="text-gray-500 hover:text-gray-700 font-medium text-sm transition-colors">
            &larr; Kembali
        </a>
    </div>

    <div class="bg-white dark:bg-night-card rounded-xl shadow-lg border border-gray-100 dark:border-night-border p-6">
        <form action="{{ route('super-admin.master.ujsimp.store') }}" method="POST">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <input type="text" name="uraian" id="uraian" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-primary focus:border-primary" required>
                </div>

                <!-- Category -->
                <div>
                    <label for="category" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Kategori</label>
                    <input type="text" name="category" id="category" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-primary focus:border-primary" required>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="urutan" id="urutan" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-primary focus:border-primary" required>
                </div>
            </div>
            ...
            <div class="mb-6">
                <label class="inline-flex items-center">
                    <input type="checkbox" name="is_active" value="1" class="rounded border-gray-300 text-primary shadow-sm focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50" checked>
                    <span class="ml-2 text-sm text-gray-600 dark:text-gray-400">Aktif</span>
                </label>
            </div>

            <div class="flex justify-end gap-3">
                <a href="{{ route('super-admin.master.ujsimp.index') }}" class="px-4 py-2 bg-gray-200 text-gray-800 rounded-lg hover:bg-gray-300 transition-colors">Batal</a>
...
<script>
    function toggleOptions() {
        ...
        if (type === 'dropdown') {
            ...
            if(document.querySelectorAll('input[name="options[]"]').length === 0) {
                addOption();
            }
        } else {
