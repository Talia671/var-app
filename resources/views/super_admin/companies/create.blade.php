...
@section('content')
<div class="mb-6">
    <h2 class="text-2xl font-bold text-gray-800 dark:text-white">Tambah Perusahaan</h2>
    <p class="text-gray-500 dark:text-gray-400 text-sm mt-1">Tambah data perusahaan baru ke sistem</p>
</div>
...
<div class="bg-white dark:bg-night-card rounded-xl shadow-lg border border-gray-100 dark:border-night-border overflow-hidden p-6">
    <form action="{{ route('super-admin.companies.store') }}" method="POST">
        @csrf
        <div class="mb-4">
            <label for="name" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Nama Perusahaan</label>
        ...
        <div class="flex gap-4">
            ...
            <a href="{{ route('super-admin.companies.index') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-800 font-medium rounded-lg text-sm px-4 py-2.5 transition-colors">
                Batal
            </a>
        </div>
    </form>
