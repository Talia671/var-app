...
<div class="flex justify-between items-center mb-6">
    <a href="{{ route('admin.ranmor.index') }}" 
    ...
    <a href="{{ route('admin.ranmor.preview', $document->id) }}" target="_blank"
    ...
    <div class="p-8 md:p-12">
        ...
        <div class="flex justify-between items-center mb-8 border-b-2 border-gray-800 pb-4">
            ...
            <div class="w-1/3 flex flex-col items-end">
                ...
                <span class="bg-gray-100 text-gray-800 text-xs font-bold px-2 py-1 border border-gray-400">CEK FISIK</span>
            </div>
        </div>

        <!-- TITLE -->
        <div class="text-center mb-8">
            <h1 class="text-xl font-bold text-gray-900 dark:text-white uppercase decoration-2 underline underline-offset-4">HASIL PEMERIKSAAN RANMOR/ALAT BERAT</h1>
        </div>
        ...
        <div class="grid grid-cols-1 gap-y-4 text-sm text-gray-800 dark:text-gray-200 mb-8">
                ...
                <div class="col-span-2 flex items-center gap-8">
                    <label class="flex items-center space-x-2">
                        <span class="w-4 h-4 border border-gray-400 flex items-center justify-center">
                            {{ str_contains($document->zona, '1') ? '✓' : '' }}
                        </span>
                        <span>ZONA 1</span>
                    </label>
                    <label class="flex items-center space-x-2">
                        <span class="w-4 h-4 border border-gray-400 flex items-center justify-center">
                            {{ str_contains($document->zona, '2') ? '✓' : '' }}
                        </span>
                        <span>ZONA 2</span>
                    </label>
                </div>
            ...
            <div class="grid grid-cols-3 gap-4 border-b border-gray-100 dark:border-gray-700 pb-2">
                <div class="font-bold">NOMOR POLISI / SERTIFIKAT</div>
                <div class="col-span-2 uppercase">: {{ $document->no_pol }}</div>
            </div>

            <div class="grid grid-cols-3 gap-4 border-b border-gray-100 dark:border-gray-700 pb-2">
                <div class="font-bold">NOMOR LAMBUNG</div>
                <div class="col-span-2 uppercase">: {{ $document->no_lambung }}</div>
            </div>

            <div class="grid grid-cols-3 gap-4 border-b border-gray-100 dark:border-gray-700 pb-2">
                <div class="font-bold">TAHUN PEMBUATAN / WARNA</div>
                <div class="col-span-2 uppercase">: {{ $document->tahun_pembuatan ?? '-' }} / {{ $document->warna ?? '-' }}</div>
            </div>

            <div class="grid grid-cols-3 gap-4 border-b border-gray-100 dark:border-gray-700 pb-2">
                <div class="font-bold">PERUSAHAAN / DEPARTEMEN</div>
                <div class="col-span-2 uppercase">: {{ $document->perusahaan ?? '-' }}</div>
            </div>

            <div class="grid grid-cols-3 gap-4 border-b border-gray-100 dark:border-gray-700 pb-2">
                <div class="font-bold">MERK KENDARAAN</div>
                <div class="col-span-2 uppercase">: {{ $document->merk_kendaraan ?? '-' }}</div>
            </div>
            ...
            <div class="grid grid-cols-3 gap-4 border-b border-gray-100 dark:border-gray-700 pb-2">
                <div class="font-bold">PENGEMUDI / OPERATOR</div>
                <div class="col-span-2 uppercase">: {{ $document->pengemudi }}</div>
            </div>
            ...
            <div class="grid grid-cols-3 gap-4 border-b border-gray-100 dark:border-gray-700 pb-2">
                <div class="font-bold">NOMOR SIMPER / SIOPER</div>
            ...
            <div class="grid grid-cols-3 gap-4 border-b border-gray-100 dark:border-gray-700 pb-2">
                <div class="font-bold">TANGGAL PERIKSA</div>
                <div class="col-span-2 uppercase">: {{ $document->tanggal_periksa ? $document->tanggal_periksa->format('d F Y') : '-' }}</div>
            </div>
        </div>
            ...
            <table class="w-full border-collapse border border-gray-300 dark:border-gray-600 text-sm">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="border border-gray-300 dark:border-gray-600 p-2 text-gray-700 dark:text-gray-300">{{ $finding->uraian }}</td>
                    </tr>
                    @empty
                    <tr>
                        <td class="border border-gray-300 dark:border-gray-600 p-2 text-center text-gray-700 dark:text-gray-300">-</td>
                        <td class="border border-gray-300 dark:border-gray-600 p-2 text-gray-700 dark:text-gray-300 italic text-center">Tidak ada temuan</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="grid grid-cols-2 gap-8 mt-12 text-center">
            ...
            <div>
                ...
                <div class="text-sm">
                    <div class="font-bold text-gray-900 dark:text-white uppercase underline">{{ $document->creator->name ?? '(.......................)' }}</div>
            ...
            <div>
                ...
                <div class="text-sm">
                    <div class="font-bold text-gray-900 dark:text-white uppercase underline">{{ $document->pengemudi }}</div>
...
<div class="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
    <!-- APPROVE FORM -->
    <form action="{{ route('admin.ranmor.approve', $document->id) }}" method="POST" class="w-full">
        @csrf
    ...
    <!-- REJECT FORM -->
    <form action="{{ route('admin.ranmor.reject', $document->id) }}" method="POST" class="w-full">
        @csrf
