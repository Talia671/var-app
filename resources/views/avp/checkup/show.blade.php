...
<div class="checkup-wrapper">
    <div class="checkup-container">
                ...
                @foreach($document->photos as $photo)
                    <div class="photo-card group">
                        ...
                        <div class="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <form action="{{ route('avp.checkup.photo.destroy', $photo->id) }}" method="POST" onsubmit="return confirm('Hapus foto ini permanently?');">
                                @csrf
                                @method('DELETE')
        ...
        <div class="btn-action-area" style="margin-top: 50px; text-align: center; display: flex; justify-content: center; gap: 15px;">
            <a href="{{ route('avp.approval-queue') }}" class="btn-modern-back" style="background: #64748b;">
                ← Kembali
            </a>

            @if($document->workflow_status == 'verified')
                <form action="{{ route('avp.checkup.approve', $document->id) }}" method="POST" class="inline">
                    @csrf
                    <button type="submit" 
            ...
            @if($document->workflow_status == 'approved')
                <a href="{{ route('avp.checkup.preview', $document->id) }}" target="_blank" class="btn-modern-back" style="background: #3b82f6;">
                    Download PDF
                </a>
            @endif
        </div>
...
<!-- Reject Modal -->
<div id="rejectModal" class="fixed inset-0 z-50 hidden bg-black bg-opacity-95 flex items-center justify-center p-4" style="position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.5); z-index: 9999; display: none; align-items: center; justify-content: center;">
    <div class="bg-white rounded-lg p-6 max-w-md w-full">
        <h3 class="text-lg font-bold mb-4">Alasan Penolakan</h3>
        <form action="{{ route('avp.checkup.reject', $document->id) }}" method="POST">
            @csrf
            <textarea name="rejected_reason" rows="4" class="w-full border rounded p-2 mb-4" placeholder="Masukkan alasan penolakan..." required></textarea>
            <div class="flex justify-end gap-2">
                <button type="button" onclick="document.getElementById('rejectModal').style.display = 'none'" class="px-4 py-2 bg-gray-300 rounded">Batal</button>
...
<!-- Image Modal -->
<div id="imageModal" class="fixed inset-0 z-50 hidden bg-black bg-opacity-95 flex items-center justify-center p-4" onclick="closeModal()" style="position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.9); z-index: 9999; display: none; align-items: center; justify-content: center;">
    <span onclick="closeModal()" style="position: absolute; top: 20px; right: 30px; color: white; font-size: 40px; cursor: pointer;">&times;</span>
    <img id="modalImage" style="max-width: 90%; max-height: 90vh; border-radius: 4px; box-shadow: 0 0 20px rgba(0,0,0,0.5);">
</div>
