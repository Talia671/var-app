...
@section('title', 'Detail Approval UJSIMP')
...
.ujsimp-info-table td {
    border: 1px solid #dee2e6;
    padding: 8px 12px;
    font-size: 14px;
}

.ujsimp-info-table .label {
    width: 40%;
    font-weight: 600;
    color: #4a5568;
}
...
<div class="py-6">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 mb-6 border border-gray-100 dark:border-gray-700">
            ...
            <div class="flex flex-wrap gap-4" x-data="{ showRejectModal: {{ $errors->has('reason') ? 'true' : 'false' }} }">
                <form action="{{ route('avp.ujsimp.approve', $test->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menyetujui dokumen ini?');">
                    @csrf
                ...
                <div x-show="showRejectModal" 
                    ...
                    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                        ...
                        <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                            <form action="{{ route('avp.ujsimp.reject', $test->id) }}" method="POST">
                                @csrf
                                <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                    <div class="sm:flex sm:items-start">
                                        ...
                                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                            ...
                                            <div class="mt-4">
                                                <textarea name="reason" rows="4" class="w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-white focus:ring-red-500 focus:border-red-500 shadow-sm" placeholder="Berikan alasan mengapa dokumen ini ditolak..." required>{{ old('reason') }}</textarea>
                                                @error('reason')
        ...
        @elseif(in_array($test->workflow_status, ['approved', 'rejected']))
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="ujsimp-wrapper">
                    <div class="ujsimp-container">
                        ...
                        <div class="ujsimp-title">
                            HASIL UJIAN PRAKTEK SIMPER / SIOPER
                        </div>

                        <!-- INFO TABLE -->
                        <table class="ujsimp-info-table">
                            <tr>
                        ...
                        <div class="recommendation-box">
                            <span style="color: #64748b; font-weight: 600;">STATUS KELULUSAN:</span>
                            <br>
                            @php
                    ...
                    <div class="relative pl-8 space-y-10 before:absolute before:left-3 before:top-2 before:bottom-2 before:w-0.5 before:bg-gray-100 dark:before:bg-gray-700">
                        ...
                        <div class="relative">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">By <span class="font-semibold text-gray-700 dark:text-gray-300">{{ $test->submitter->name ?? ($test->checker->name ?? 'Checker') }}</span></p>
                                <p class="text-[10px] text-gray-400 mt-1 uppercase font-bold">{{ $test->submitted_at ? $test->submitted_at->format('d M Y H:i') : ($test->created_at->format('d M Y H:i')) }}</p>
                            </div>
                        </div>

                        <!-- STEP 2: VERIFIED -->
                        <div class="relative">
                            @php $isVerified = $test->verified_at; @endphp
                            ...
                            <div class="{{ $isVerified ? '' : 'opacity-50' }}">
                                ...
                                @if($isVerified)
                                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">By <span class="font-semibold text-gray-700 dark:text-gray-300">{{ $test->verifier->name ?? 'Admin' }}</span></p>
                                    <p class="text-[10px] text-gray-400 mt-1 uppercase font-bold">{{ $test->verified_at->format('d M Y H:i') }}</p>
                                @else
                        ...
                        <div class="relative">
                            @php 
                                $isFinalized = in_array($test->workflow_status, ['approved', 'rejected']);
                                $isApproved = $test->workflow_status === 'approved';
                                $isRejected = $test->workflow_status === 'rejected';
                                $finalUser = $isApproved ? $test->approver : ($isRejected ? $test->rejecter : null);
                                $finalAt = $isApproved ? $test->approved_at : ($isRejected ? $test->rejected_at : null);
                            @endphp
                            ...
                            <div class="{{ $isFinalized ? '' : 'opacity-50' }}">
                                ...
                                @if($isFinalized)
                                    ...
                                    @if($isRejected && $test->rejected_reason)
                                        <div class="mt-2 p-2 bg-red-50 dark:bg-red-900/20 rounded border border-red-100 dark:border-red-800">
                                            ...
                                            <p class="text-[11px] text-red-700 dark:text-red-300 italic">"{{ $test->rejected_reason }}"</p>
                                        </div>
                                    @endif
                                @else
